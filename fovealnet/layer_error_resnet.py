...
from timm_vit import ResNetFoveated
...
model_path = '/home/ubuntu/gaussian_splatting_with_eye_tracking/fovealnet/results_epoch_resnet/epoch_%d/model_epoch_%d.pt'%(epoch_idx, epoch_idx)
...
model = ResNetFoveated(backbone_name='resnet34')
...
if not os.path.exists('error_stat_resnet'):
    os.makedirs('error_stat_resnet')
# Save the errors array to a file
np.save('error_stat_resnet/prediction_errors.npy', errors)
...
ax.legend()
plt.tight_layout()
plt.savefig('error_stat_resnet/prediction_errors.png')
plt.show()
