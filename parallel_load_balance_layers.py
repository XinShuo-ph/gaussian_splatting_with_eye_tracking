...
# and instead of tuning topk, tune number of layers
# used the training set in this script, since the eye-tracking prediction on test set is still not good enough
...
num_images = 100  # Adjust based on your data
max_steps = 7  # Number of fovea steps
...
parser.add_argument("--gaze_r2", default=1000, type=float)
parser.add_argument("--gaze_r3", default=300, type=float)
parser.add_argument("--gaze_r4", default=150, type=float)
...
def render_mpi(viewpoint_camera, pc : GaussianModel, pipe, bg_color : torch.Tensor, scaling_modifier = 1.0, override_color = None, 
    ...
    if gaze_radius_ranges_file is not None:
        ...
        # lower bound means 99% of the rendered pixels would be useful (inside ground truth)
    ...
    # instead of using foveaStep to control the rendering quality, use gaze_r to control and set the foveaStep to 5
    foveaStep=5
    ...
    for img_idx in [0]:
        ...
        while foveaLayer < args.final_layer:
         # i.e. render until reach highest foveal level
            ...
            if foveaLayer != local_fovealnet_level_buffer[img_idx]:
            ...
            if foveaLayer == local_fovealnet_level_buffer[img_idx]:
                ...
                time.sleep(0.005)
                continue
            # when tuning layers, do not skip any fovea step
            if local_fovealnet_level_buffer[img_idx]>foveaLayer+1:
                foveaLayer += 1
            else:
                foveaLayer = local_fovealnet_level_buffer[img_idx]
            mid_x = (pix_x*ratio) / 2
            mid_y = (pix_y*ratio) / 2
            ...
            if gaze_render_radii_file is not None:
                ...
                if foveaLayer >= args.final_layer:
                    gaze_r2_cur = gaze_r2_u[foveaLayer-1]
                    gaze_r3_cur = gaze_r3_u[foveaLayer-1]
                    gaze_r4_cur = gaze_r4_u[foveaLayer-1]
            ...
            rendered_image0, radii, geomBuffer, binningBuffer, imageBuffer = rawrasterizer.apply(
                ...
                cov3D_precomp,
                    ...
                    gaze_x,  # gaze direction x
                    gaze_y,  # gaze direction y
                    gaze_r2_cur,gaze_r3_cur,gaze_r4_cur,  # radii of the foveal level 2,3,4
                    percentile_r2, percentile_r3, percentile_r4,  # percentiles of the foveal level 2,3,4
                    out_color_precomp,
                    # radii_precomp,
                    # means2D_precomp,
...
if rank == 0:  # Gaussian Splatting process
    ...
    for seq_id in range(args.eye_image_sequence_id_start, args.eye_image_sequence_id_end ):
        ...
        for img_idx in range(100):
            image_name = f"{img_idx:03d}.png"
            image_path = os.path.join(sequence_folder, image_name)
            image = load_image(image_path).to(device)

            if args.tune_layer_iter >=0 and img_idx >0: # if tune layers, we do not continue running the other 99 images in the experiment
                break
